...
class ArcGISMCPServer:
    def __init__(self):
        ...
        logger.info(
            f"ArcGIS configuration loaded - Server: {self.server_url}, Portal: {self.portal_url}"
        )
        ...
        async def handle_call_tool(
            ...
            try:
                if name == "list_arcgis_services":
                    ...
                    return [
                        TextContent(type="text", text=json.dumps(services, indent=2))
                    ]
                    ...
                    return [
                        TextContent(type="text", text=json.dumps(details, indent=2))
                    ]
                    ...
                    return [
                        TextContent(
                            type="text",
                            text=json.dumps(
                                {"token": token, "expiration_minutes": expiration},
                                indent=2,
                            ),
                        )
                    ]
    ...
    async def get_portal_token(self, expiration: int = 60) -> Optional[str]:
        ...
        if not self.username or not self.password:
            raise ValueError(
                "AGS_USERNAME and AGS_PASSWORD environment variables are required"
            )
    ...
    async def get_service_details(
        self, service_name: str, folder: str = ""
    ) -> Dict[str, Any]:
