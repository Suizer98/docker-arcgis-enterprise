...
Currently, this deployment doesn't use Nginx as a reverse proxy. But we can add it as one of the service in docker compose if needed.
...
```
3. Run script [`create_enterprise_gdb.py`](postgres/create_enterprise_gdb.py) in postgres folder, you can rename the database name as you wish inside the script, I didn't implement variable pass in.
...
**Solution**: Add Datastore's own PostgreSQL access permissions for the actual database user:
...
### Unable to register self hosted PostgreSQL EGDB from ArcGIS Server

**Problem**: Complete error message when registering EGDB loaded with ST_GEOMETRY:
```
...
Usually in restrcited environment like government cloud VMs, they are air-gapped and internet is not accessible. 
You may refer to this [website](https://enterprise.arcgis.com/en/server/10.9.1/install/linux/silently-install-arcgis-server.htm) to do one time authorisation for your .prvc provisioning file, if you want to generate .ecp file for offline use.
